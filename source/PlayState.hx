...
//	for y, adding makes you go down, subtracting up
				...
				//note to self for x and y shit, down is add, subtract is up so the negatives is high up.
...
//	for y, adding makes you go down, subtracting up
			...
			case 'fishplace': // Jibanyan
				...
				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('fpShogun/sky', -100, 0, 0.1, 0.1);
					add(bg);
				}

				var city:BGSprite = new BGSprite('fpShogun/city', -10, 0, 0.3, 0.3);
				city.setGraphicSize(Std.int(city.width * 0.85));
				city.updateHitbox();
				add(city);
				...
				phillyTrain = new BGSprite('fpShogun/truck', 2000, 360);
				add(phillyTrain);
				...
				var street:BGSprite = new BGSprite('fpShogun/street', -40, 50);
				add(street);
		...
		timeTxt.setFormat(Paths.font("FOT-RowdyStd-EB.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("FOT-RowdyStd-EB.otf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("FOT-RowdyStd-EB.otf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Change Camera Zoom':
				var newZoom = Std.parseFloat(value1);
				FlxTween.tween(FlxG.camera, {zoom: newZoom}, Std.parseFloat(value2), {ease: FlxEase.quadOut, onComplete: function(twn:FlxTween){
		...
		for (i in 0...arrayIDs.length) {
			if(!Achievements.achievementsUnlocked[arrayIDs[i]][1]) {
				switch(arrayIDs[i]) {
					...
					case 15:
						if(Paths.formatToSongPath(SONG.song) == 'for-naughty-brats' && !usedPractice) {
							Achievements.unlockAchievement(arrayIDs[i]);
							return arrayIDs[i];
						}
				}
